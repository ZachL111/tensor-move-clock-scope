...
`tensor-move-clock-scope` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies clock behavior through layout fixtures, with stable geometry snapshots and no production deployment claims.
